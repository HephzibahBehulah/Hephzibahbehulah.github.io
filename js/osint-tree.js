/* ========================================
   ...
   For Resources, Portfolio, and Pentest pages
   ======================================== */
...
class OsintTree {
                ...
                <div class="tree-legend">
                    ...
                    <div class="legend-items">
                        <div class="legend-item">
                            <span class="legend-badge" style="background: rgba(251, 191, 36, 0.2); color: #fbbf24;">T</span>
                            <span>Tool (Local Install)</span>
                        </div>
                        <div class="legend-item">
                            <span class="legend-badge" style="background: rgba(239, 68, 68, 0.2); color: #ef4444;">D</span>
                            <span>Google Dork</span>
                        </div>
                        <div class="legend-item">
                            <span class="legend-badge" style="background: rgba(96, 165, 250, 0.2); color: #60a5fa;">R</span>
                            <span>Requires Registration</span>
                        </div>
                        <div class="legend-item">
                            <span class="legend-badge" style="background: rgba(167, 139, 250, 0.2); color: #a78bfa;">M</span>
                            <span>Manual URL Edit</span>
                        </div>
                    </div>
                </div>
    ...
    createSVG() {
        // SVG will be populated during render
    }
    ...
    renderNode(node, x, y, parentId, depth) {
        ...
        if (hasChildren && isExpanded) {
            ...
            node.children.forEach((child, index) => {
                const childNodeWidth = this.estimateWidth(child);
    ...
    estimateWidth(node) {
        ...
        const badgeWidth = node.badge ? 30 : 0;
        return Math.max(120, text.length * 7 + badgeWidth + 40);
    }
    ...
    drawConnections() {
        ...
        // Set SVG size
        svg.setAttribute('width', this.inner.scrollWidth);
        svg.setAttribute('height', this.inner.scrollHeight);
    }
    
    showModal(node) {
        document.getElementById('nodeModalTitle').innerHTML = 
            `${node.icon || ''} ${node.name}`;
        document.getElementById('nodeModalDescription').textContent = 
        ...
        if (node.url) {
            linksContainer.innerHTML += `
                <a href="${node.url}" target="_blank" class="node-modal-link">
                    <span>🌐</span>
                    <span>Visit ${node.name}</span>
                </a>
            `;
        }
        ...
        if (node.github) {
            linksContainer.innerHTML += `
                <a href="${node.github}" target="_blank" class="node-modal-link">
                    <span>🐙</span>
        ...
        if (node.links && Array.isArray(node.links)) {
            node.links.forEach(link => {
                linksContainer.innerHTML += `
                    <a href="${link.url}" target="_blank" class="node-modal-link">
                        <span>${link.icon || '🔗'}</span>
                        <span>${link.name}</span>
                    </a>
                `;
    ...
    setupControls() {
        ...
        // Modal close
        document.getElementById('nodeModalClose')?.addEventListener('click', () => {
            this.hideModal();
        });
        
        document.getElementById('nodeModalOverlay')?.addEventListener('click', () => {
            this.hideModal();
        });
